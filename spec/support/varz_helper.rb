...
module VARZHelper
  ...
  def varz_stub
    AdminUI::Utils.stub(:http_request).with(anything, nats_cloud_controller_varz, 'GET', anything) do
      OK.new(varz_cloud_controller)
    end

    AdminUI::Utils.stub(:http_request).with(anything, nats_dea_varz, 'GET', anything) do
      OK.new(varz_dea)
    end

    AdminUI::Utils.stub(:http_request).with(anything, nats_health_manager_varz, 'GET', anything) do
      OK.new(varz_health_manager)
    end

    AdminUI::Utils.stub(:http_request).with(anything, nats_provisioner_varz, 'GET', anything) do
      OK.new(varz_provisioner)
    end

    AdminUI::Utils.stub(:http_request).with(anything, nats_router_varz, 'GET', anything) do
      OK.new(varz_router)
    end
  end
